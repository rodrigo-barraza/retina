    ...
    for (const section of MODEL_SECTIONS) {
        ...
        for (const [provider, models] of Object.entries(providers)) {
            for (const m of models) {
                // Use provider:name as unique key to avoid duplicates
                // (same model name may appear in multiple sections, e.g. gemini-3-flash in textToText and audioToText)
                const key = `${provider}:${m.name}`;
                if (!modelsMap.has(key)) {
                    modelsMap.set(key, { ...m, provider });
                } else {
                    // Merge additional data (e.g. arena, pricing) from other sections
...
export default function ModelsPage() {
    ...
    const fetchModels = useCallback(async () => {
        try {
            ...
            // Flatten all config models into a single list
            const flat = flattenConfigModels(config);

            // Build a lookup of LM Studio API models for load/unload actions
            const lmApiModels = (lmData.models || []).filter((m) => m.type === "llm");
            const lmApiMap = new Map(lmApiModels.map((m) => [m.key, m]));

            // Merge LM Studio API data (loaded status, instances) into the flat list
    ...
    const renderActions = (model) => {
        // Only show load/unload for LM Studio models
